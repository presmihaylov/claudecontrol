...
func TestBroadcastCheckIdleJobs(t *testing.T) {
	t.Run("success_single_organization", func(t *testing.T) {
		...
		mockWSClient.On("SendMessage", "ws-001", mock.MatchedBy(func(msg interface{}) bool {
		...
		mockWSClient.On("SendMessage", "ws-002", mock.MatchedBy(func(msg interface{}) bool {
			// Verify it's a CheckIdleJobs message
			baseMsg, ok := msg.(models.BaseMessage)
			return ok && baseMsg.Type == models.MessageTypeCheckIdleJobs
		})).Return(nil)
	...
	t.Run("organizations_service_error", func(t *testing.T) {
		// Setup
		ctx := context.Background()
		mockAgentsService := new(agents.MockAgentsService)
		mockWSClient := new(socketio.MockSocketIOClient)
	...
	t.Run("agents_service_error", func(t *testing.T) {
		// Setup
		ctx := context.Background()
		mockAgentsService := new(agents.MockAgentsService)
		mockWSClient := new(socketio.MockSocketIOClient)
	...
	t.Run("send_message_error", func(t *testing.T) {
		// Setup
		ctx := context.Background()
		mockAgentsService := new(agents.MockAgentsService)
		mockWSClient := new(socketio.MockSocketIOClient)
		...
		mockOrganizationsService.On("GetAllOrganizations", ctx).
			Return([]*models.Organization{organization}, nil)
		mockWSClient.On("GetClientIDs").
			Return([]string{"ws-001"})
		mockAgentsService.On("GetConnectedActiveAgents", ctx, "org-456", []string{"ws-001"}).
			Return([]*models.ActiveAgent{agent1}, nil)

		// Expect SendMessage to fail
		mockWSClient.On("SendMessage", "ws-001", mock.MatchedBy(func(msg interface{}) bool {
			// Verify it's a CheckIdleJobs message
			baseMsg, ok := msg.(models.BaseMessage)
			return ok && baseMsg.Type == models.MessageTypeCheckIdleJobs
