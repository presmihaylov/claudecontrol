...
func (r *PostgresAgentsRepository) GetAgentByJobID(
	...
	query := fmt.Sprintf(`
		...
		WHERE aja.job_id = $1 AND a.organization_id = $2
...
func (r *PostgresAgentsRepository) AssignAgentToJob(ctx context.Context, assignment *models.AgentJobAssignment) error {
	...
	insertColumns := []string{"id", "agent_id", "job_id", "assigned_at"}
	columnsStr := strings.Join(insertColumns, ", ")
	...
	err := r.db.QueryRowxContext(ctx, query, assignment.ID, assignment.AgentID, assignment.JobID).
...
func (r *PostgresAgentsRepository) UnassignAgentFromJob(
	...
	query := fmt.Sprintf(`
		...
		WHERE agent_id = $1 AND job_id = $2`, r.schema)

	result, err := r.db.ExecContext(ctx, query, agentID, jobID)
	if err != nil {
...
func (r *PostgresAgentsRepository) GetActiveAgentJobAssignments(
	...
	var jobIDs []string
	err := r.db.SelectContext(ctx, &jobIDs, query, agentID)
	if err != nil {
