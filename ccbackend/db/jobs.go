...
func (r *PostgresJobsRepository) CreateJob(job *models.Job) error {
	query := fmt.Sprintf(`
		INSERT INTO %s.jobs (id, slack_thread_ts, slack_channel_id, slack_integration_id, status, created_at, updated_at) 
		VALUES ($1, $2, $3, $4, $5, NOW(), NOW()) 
		RETURNING id, slack_thread_ts, slack_channel_id, slack_integration_id, status, created_at, updated_at`, r.schema)

	err := r.db.QueryRowx(query, job.ID, job.SlackThreadTS, job.SlackChannelID, job.SlackIntegrationID, job.Status).StructScan(job)
	if err != nil {
...
func (r *PostgresJobsRepository) GetJobByID(id uuid.UUID, slackIntegrationID string) (*models.Job, error) {
	query := fmt.Sprintf(`
		SELECT id, slack_thread_ts, slack_channel_id, slack_integration_id, status, created_at, updated_at 
		FROM %s.jobs 
...
func (r *PostgresJobsRepository) UpdateJob(job *models.Job) error {
	query := fmt.Sprintf(`
		UPDATE %s.jobs 
		SET slack_thread_ts = $2, slack_channel_id = $3, status = $4, updated_at = NOW() 
		WHERE id = $1 AND slack_integration_id = $5
		RETURNING id, slack_thread_ts, slack_channel_id, slack_integration_id, status, created_at, updated_at`, r.schema)

	err := r.db.QueryRowx(query, job.ID, job.SlackThreadTS, job.SlackChannelID, job.Status, job.SlackIntegrationID).StructScan(job)
	if err != nil {
...
func (r *PostgresJobsRepository) GetIdleJobs(idleMinutes int) ([]*models.Job, error) {
	query := fmt.Sprintf(`
		SELECT j.id, j.slack_thread_ts, j.slack_channel_id, j.slack_integration_id, j.status, j.created_at, j.updated_at 
		FROM %s.jobs j
...
func (r *PostgresJobsRepository) GetJobsWithQueuedMessages(slackIntegrationID string) ([]*models.Job, error) {
	query := fmt.Sprintf(`
		SELECT DISTINCT j.id, j.slack_thread_ts, j.slack_channel_id, j.slack_integration_id, j.status, j.created_at, j.updated_at 
		FROM %s.jobs j
