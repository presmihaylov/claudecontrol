...
func (s *AgentsService) CreateActiveAgent(wsConnectionID string, assignedJobID *uuid.UUID) (*models.ActiveAgent, error) {
...
func (s *AgentsService) DeleteActiveAgentByWsConnectionID(wsConnectionID string) error {
	...
	if err := s.agentsRepo.DeleteActiveAgent(agent.ID); err != nil {
...
func (s *AgentsService) DeleteActiveAgent(id uuid.UUID) error {
	...
	if err := s.agentsRepo.DeleteActiveAgent(id); err != nil {
		return fmt.Errorf("failed to delete active agent: %w", err)
	}
...
func (s *AgentsService) GetAgentByID(id uuid.UUID) (*models.ActiveAgent, error) {
	...
	agent, err := s.agentsRepo.GetAgentByID(id)
	if err != nil {
...
func (s *AgentsService) GetAvailableAgents() ([]*models.ActiveAgent, error) {
	log.Printf("📋 Starting to get available agents")

	agents, err := s.agentsRepo.GetAvailableAgents()
	if err != nil {
