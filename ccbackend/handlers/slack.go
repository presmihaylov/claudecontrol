...
func (h *SlackWebhooksHandler) HandleSlackEvent(w http.ResponseWriter, r *http.Request) {
	...
	slackIntegrationOpt, err := h.slackIntegrationsService.GetSlackIntegrationByTeamID(r.Context(), teamID)
	if err != nil {
	...
	if !slackIntegrationOpt.IsPresent() {
		log.Printf("❌ Slack integration not found for team %s", teamID)
		http.Error(w, "integration not found", http.StatusNotFound)
		return
	}
	slackIntegration := slackIntegrationOpt.MustGet()
