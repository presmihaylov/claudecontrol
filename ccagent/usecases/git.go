...
func (g *GitUseCase) handlePRCreationOrUpdate(branchName, slackThreadLink string) (*AutoCommitResult, error) {
	...
	if hasExistingPR {
		log.Info("✅ Existing PR found for branch %s - updating title and description", branchName)
		...
		// Update PR title and description with new changes
		if err := g.updateExistingPR(branchName, slackThreadLink); err != nil {
			log.Error("❌ Failed to update existing PR title/description: %v", err)
			// Continue without failing the entire operation
		} else {
			log.Info("✅ Successfully updated existing PR title and description")
		}
...
func (g *GitUseCase) updateExistingPR(branchName, slackThreadLink string) error {
	log.Info("📋 Starting to update existing PR for branch: %s", branchName)
	...
	// Generate updated PR title with context
	newTitle, err := g.generateUpdatedPRTitleWithClaude(branchName, currentTitle)
	if err != nil {
		return fmt.Errorf("failed to generate updated PR title: %w", err)
	}

	// Generate updated PR description with context
	newDescription, err := g.generateUpdatedPRBodyWithClaude(branchName, currentDescription, slackThreadLink)
	if err != nil {
		return fmt.Errorf("failed to generate updated PR description: %w", err)
	}

	// Update PR title if it changed meaningfully
	if shouldUpdateTitle(currentTitle, newTitle) {
		if err := g.gitClient.UpdatePRTitle(branchName, newTitle); err != nil {
			return fmt.Errorf("failed to update PR title: %w", err)
		}
		log.Info("✅ Updated PR title from '%s' to '%s'", currentTitle, newTitle)
	} else {
		log.Info("ℹ️ PR title remains unchanged: %s", currentTitle)
	}

	// Update PR description (always update since it includes new commits)
	if err := g.gitClient.UpdatePRDescription(branchName, newDescription); err != nil {
...
// shouldUpdateTitle determines if the title should be updated based on meaningful differences
func shouldUpdateTitle(currentTitle, newTitle string) bool {
	// Don't update if titles are identical
	if currentTitle == newTitle {
		return false
	}

	// Don't update if new title is just whitespace
...
func (g *GitUseCase) generateUpdatedPRTitleWithClaude(branchName, currentTitle string) (string, error) {
	...
	prompt := fmt.Sprintf(`I'm updating an existing pull request for Git branch: "%s"

CURRENT PR TITLE: "%s"

Here are ALL commits made on this branch (including new ones since PR creation):
%s
...
Generate an UPDATED pull request title. IMPORTANT REQUIREMENTS:
- Only change the title if the new changes make it obsolete or incomplete
- If the current title still accurately describes the work, keep it unchanged
- If updating, make it ADDITIVE to the current scope, don't replace it entirely
- Maximum 40 characters (STRICT LIMIT)
- Start with action verb (Add, Fix, Update, Improve, etc.)
- Be concise and specific about the OVERALL work being done
- Don't mention "Claude", "agent", or implementation details
- Base the title on the complete set of changes shown above

Examples of good updates:
- "Fix auth validation" → "Fix auth validation and add middleware"
- "Add user service" → "Add user service with validation"
- "Update API format" → "Update API format and error handling"

If the current title already covers all the work, respond with it unchanged.

Respond with ONLY the updated title, nothing else.`, branchName, currentTitle, commitInfo, diffSummary, diffContent)
...
func (g *GitUseCase) generateUpdatedPRBodyWithClaude(branchName, currentDescription, slackThreadLink string) (string, error) {
	...
	// Remove existing footer from current description to get clean content
	cleanCurrentDescription := g.removePRFooter(currentDescription)
...
Generate an UPDATED pull request description. IMPORTANT REQUIREMENTS:
- Make it ADDITIVE to the current description, don't replace it entirely
- Keep the existing summary structure but update it to reflect ALL work done
- Add any new changes to the existing sections in a natural way
- If new functionality was added, incorporate it into the summary
- Maintain the same professional tone and structure as the current description
- Use proper markdown formatting
- Keep it concise but comprehensive

Structure should remain:
- ## Summary: Update to reflect all changes (keep existing + add new)
- ## Why: Update reasoning if scope expanded

IMPORTANT: 
- Do NOT include any "Generated with Claude Control" or similar footer text. I will add that separately.
- Focus on the complete, updated scope of work
- Make it feel like a natural evolution of the original description
- Do NOT include any introductory text like "Here is your updated description"

Respond with ONLY the updated PR description in markdown format, nothing else.`, branchName, cleanCurrentDescription, commitInfo, diffSummary, diffContent)
...
// removePRFooter removes the Claude Control footer from a PR description
func (g *GitUseCase) removePRFooter(description string) string {
...
func (g *GitUseCase) ValidateAndRestorePRDescriptionFooter(slackThreadLink string) error {
	...
	expectedFooter := fmt.Sprintf("Generated by [Claude Control](https://claudecontrol.com) from this [slack thread](%s)", slackThreadLink)
