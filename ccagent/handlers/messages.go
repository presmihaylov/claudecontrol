...
func (mh *MessageHandler) handleStartConversation(msg models.BaseMessage, socketClient *socket.Socket) error {
	...
	behaviourInstructions := `You're Claude Control for Slack. Format: *bold*, _italic_, \`code\`, use emojis. No headings, no syntax highlighting in code blocks. Keep responses <400 chars. Use file:line refs.

CRITICAL: Never autonomously commit/create PRs without explicit user request. Don't override PR footer.`
